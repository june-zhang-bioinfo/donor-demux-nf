...
# --- 2. Load Data ---
cat(paste0("Processing run: ", run_name, "\n"))
...
clusters_pca <- abs(clusters_pca)
clusters_pca <- log2(clusters_pca + 1)
...
# --- 6. PCA Visualization ---
has_pc3 <- "PC3" %in% colnames(PCi)
pdf_width <- if(has_pc3) 10 else 7
pdf_height <- if(has_pc3) 6 else 3

pdf(file = "PCA_plots.pdf", width = pdf_width, height = pdf_height)

p1 <- ggplot(PCi, aes(x = PC1, y = PC2, col = sex)) +
  geom_point(size = 1, alpha = 0.7) + theme_classic() +
  scale_color_manual(values = c("#F8A19FFF", "#325A9BFF", "#27e627d4")) + ggtitle(paste(run_name, "by Sex"))

p4 <- ggplot(PCi, aes(x = PC1, y = PC2, col = assignment)) +
  geom_point(size = 1, alpha = 0.5) + theme_classic() +
  scale_color_manual(values = DiscretePalette_scCustomize(20, "glasbey", shuffle_pal = TRUE)) + ggtitle(paste(run_name, "by Donor"))

if (has_pc3) {
  p2 <- ggplot(PCi, aes(x = PC1, y = PC3, col = sex)) + geom_point(size = 1, alpha = 0.7) + theme_classic() + scale_color_manual(values = c("#F8A19FFF", "#325A9BFF", "#27e627d4"))
  p3 <- ggplot(PCi, aes(x = PC2, y = PC3, col = sex)) + geom_point(size = 1, alpha = 0.7) + theme_classic() + scale_color_manual(values = c("#F8A19FFF", "#325A9BFF", "#27e627d4"))
  p5 <- ggplot(PCi, aes(x = PC1, y = PC3, col = assignment)) + geom_point(size = 1, alpha = 0.5) + theme_classic() + scale_color_manual(values = DiscretePalette_scCustomize(20, "glasbey", shuffle_pal = TRUE))
  p6 <- ggplot(PCi, aes(x = PC2, y = PC3, col = assignment)) + geom_point(size = 1, alpha = 0.5) + theme_classic() + scale_color_manual(values = DiscretePalette_scCustomize(20, "glasbey", shuffle_pal = TRUE))
  print(plot_grid(p1, p2, p3, p4, p5, p6, nrow = 2))
} else {
  # If only 2 patients, just show the PC1 vs PC2 comparison
  print(plot_grid(p1, p4, nrow = 1))
}
dev.off()

# --- 7. Sex Gene Expression ---
pdf(file = "SexGene_plots.pdf", width = 15, height = 9)
...
create_gene_plots <- function(PCx, PCy, genes_list){
  plots <- list()
  for(g in genes_list){
    plots[[g]] <- ggplot(PCi_plot, aes_string(x = PCx, y = PCy)) +
      geom_point(aes_string(col = g), alpha = 0.7, size = 1) +
      theme_classic() + scale_colour_gradient2(low = "orange", mid = "grey", high = "blue")
  }
  return(plots)
}

print(plot_grid(plotlist = create_gene_plots("PC1", "PC2", genes), nrow = 2))
if (has_pc3) {
  print(plot_grid(plotlist = create_gene_plots("PC1", "PC3", genes), nrow = 2))
  print(plot_grid(plotlist = create_gene_plots("PC2", "PC3", genes), nrow = 2))
}
dev.off()
